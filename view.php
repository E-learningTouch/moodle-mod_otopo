...
if ($o) {
    ...
    [
        $course,
        $cm,
    ]               = get_course_and_cm_from_cmid($id);
    ...
    if ($action === 'grader') {
        ...
        if ($action === 'edit') {
            ...
            if ($object == 'individual') {
                ...
                $filterset->add_filter(new integer_filter('session', filter::JOINTYPE_DEFAULT, [ 0 ]));
                // ToDo - unused?
            }
            ...
            if ($object == 'individual') {
                ...
                if (has_capability('mod/otopo:exportresults', $modulecontext)) {
                    ...
                    $content .= '<a id="export-csv" class="btn-otopo ml-2 mr-5 no-print" href="#">'.get_string('exportascsv', 'otopo').'</a>';
                } else {
        ...
        $body .= $output->render(
            new mod_otopo\output\view_page(
                $cm,
                $moduleinstance,
                $action,
                $object,
...
if (has_capability('mod/otopo:fill', $modulecontext)) {
    ...
    $body .= $output->render(
        new mod_otopo\output\view_fill_page(
            $cm,
            $moduleinstance,
            $action,
            $session,
